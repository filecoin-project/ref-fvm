...
where
    S: BlockStore,
{
    pub fn new(store: &'db S, version: StateTreeVersion) -> Result<Self> {
    ...
    pub fn new_from_root(store: &'db S, c: &Cid) -> Result<Self> {
    ...
    pub fn revert_to_snapshot(&mut self) -> Result<()> {
    ...
    where
        F: FnMut(Address, &ActorState) -> Result<()>,
        S: BlockStore,
    {
