...
fn selfdestruct_is_noop_under_authority_context() {
    ...
    let beneficiary20 = [
        0xBA, 0xAD, 0xF0, 0x0D, 0xAA,
        0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
        0xBB, 0xBB, 0xBB, 0xBB, 0xBB,
        0xCC, 0xCC, 0xCC, 0xCC, 0xCC,
    ];
    let del = fevm::create_contract(&mut h.tester, &mut owner, &selfdestruct_delegate(beneficiary20)).unwrap();
    assert!(del.msg_receipt.exit_code.is_success());
    let delegate_eth = del.msg_receipt.return_data.deserialize::<fevm::CreateReturn>().unwrap().eth_address.0;
    ...
    let _inv = fevm::invoke_contract(&mut h.tester, &mut owner, caller_addr, &[], fevm::DEFAULT_GAS).unwrap();
