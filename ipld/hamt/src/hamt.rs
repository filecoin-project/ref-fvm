    ...
    H: HashAlgorithm,
{
    ...
    /// unless all values in the HAMT are exhausted. The `start_at` cursor is only valid if it is
    /// generated by a previous call of `for_each_ranged` on the same HAMT and no data has been
    /// modified in the HAMT between calls. It is the responsibility of the caller to ensure that
    /// cursors are not reused after the HAMT has been modified.
    ///
    /// This function will constrain all values to be of the same type
    ///
    ...
    /// map.for_each_ranged(&LeafCursor::start(), 1, |_, v: &u64| {
    ///    iteration_count += 1;
    ///    Ok(())
    /// }).unwrap();
