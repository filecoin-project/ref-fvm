...
extern "C" {
    ...
    pub fn root(cid: *mut u8, cid_max_len: u32) -> super::SyscallResult1<u32>;

    /// Sets the root CID for the calling actor. The new root must be in the reachable set.
    pub fn set_root(cid: *const u8) -> super::SyscallResult0;

    /// Gets the current balance for the calling actor.
    pub fn current_balance() -> super::SyscallResult2<u64, u64>;

    /// Destroys the calling actor, sending its current balance
    /// to the supplied address, which cannot be itself.
    pub fn self_destruct(addr_off: *const u8, addr_len: u32) -> super::SyscallResult0;
}
