...
pub fn put(mh_code: u64, mh_size: u32, codec: u64, data: &[u8]) -> SyscallResult<Cid> {
    unsafe {
        let id = sys::ipld::create(codec, data.as_ptr(), data.len() as u32).into_result()?;
...
pub fn get(cid: &Cid) -> SyscallResult<Vec<u8>> {
    unsafe {
        ...
        let (id, _, size) = sys::ipld::open(cid_buf.as_mut_ptr()).into_result()?;
...
pub fn get_block(id: BlockId, size: Option<u32>) -> SyscallResult<Vec<u8>> {
    let size = match size {
        Some(size) => size,
        None => unsafe { sys::ipld::stat(id).into_result().map(|(_, size)| size)? },
    };
    let mut block = Vec::with_capacity(size as usize);
    unsafe {
        let bytes_read = sys::ipld::read(id, 0, block.as_mut_ptr(), size).into_result()?;
...
pub fn put_block(codec: Codec, data: &[u8]) -> SyscallResult<BlockId> {
    unsafe { sys::ipld::create(codec, data.as_ptr(), data.len() as u32).into_result() }
}
