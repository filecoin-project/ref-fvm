...
// The following contracts were copied here to exercise some of the scenarios described in
// https://0age.medium.com/the-promise-and-the-peril-of-metamorphic-contracts-9eb8b8413c5e
//
// In particular I'll start with the one that seems to make the actual contracts that can be swapped the simplest to implement,
// by using the transient contract to create them and then selfdestruct to forward the funds.
//
// The contracts needed will be:
// * the `MetamorphicContractFactory`
// * the `TransientContract`
//
// Two contracts to deploy; these will be put in `SelfDestruct.sol`:
// * a contract `Cocoon` we deploy first using the factory and the transient; a contract which can self-destruct
// * a contract `Butterfly` we replace the `Coccoon` with, after it self-destructed.
...
// https://github.com/0age/metamorphic/blob/master/contracts/TransientContract.sol
